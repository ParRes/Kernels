...
def codegen(src,pattern,stencil_size,radius,W,model):
    if (model=='openmp'):
        src.write('void '+pattern+str(radius)+'(const int n, const double * restrict A, double * restrict B) {\n')
        src.write('    OMP_FOR()\n')
        ...
        src.write('void '+pattern+str(radius)+'(const int n, const int gs, const double * restrict A, double * restrict B) {\n')
        src.write('    OMP_TASKLOOP( firstprivate(n) shared(A,B) grainsize(gs) )\n')
        src.write('    for (int i='+str(radius)+'; i<n-'+str(radius)+'; i++) {\n')
        src.write('      OMP_SIMD\n')
        src.write('      for (int j='+str(radius)+'; j<n-'+str(radius)+'; j++) {\n')
    elif (model=='cilk'):
        ...
        src.write('export void '+pattern+str(radius)+'(uniform const int n, uniform const double A[], uniform double B[]) {\n')
        src.write('    for (uniform int i='+str(radius)+'; i<n-'+str(radius)+'; i++) {\n')
        src.write('      for (uniform int j='+str(radius)+'; j<n-'+str(radius)+'; j++) {\n')
    else:
        src.write('void '+pattern+str(radius)+'(const int n, const double * restrict A, double * restrict B) {\n')
        src.write('    for (int i='+str(radius)+'; i<n-'+str(radius)+'; i++) {\n')
        src.write('      PRAGMA_SIMD\n')
    ...
    for j in range(0,2*radius+1):
        for i in range(0,2*radius+1):
            if ( W[j][i] != 0.0):
                k+=1
                src.write('+A[(i+'+str(j-radius)+')*n+(j+'+str(i-radius)+')] * '+str(W[j][i]))
                if (k<kmax): src.write('\n')
