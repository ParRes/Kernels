...
case "$os" in
    Darwin)
        ...
        which glibtoolize
        #glibtool --version
        ln -s `which glibtool` ${CI_ROOT}/bin/libtool
        ln -s `which glibtoolize` ${CI_ROOT}/bin/libtoolize
        ;;
    Linux)
        MAKE_JNUM=2
        M4_VERSION=1.4.17
        ...
        if [ -f ${BIN} ] ; then
          ...
          cd ${CI_ROOT}/${TDIR}
          ./configure CC=cc --prefix=${CI_ROOT} && make -j ${MAKE_JNUM} && make install
          if [ "x$?" != "x0" ] ; then
            echo FAILURE 1
            exit
          fi
        ...
        if [ -f ${BIN} ] ; then
          ...
          cd ${CI_ROOT}/${TDIR}
          ./configure CC=cc --prefix=${CI_ROOT} M4=${CI_ROOT}/bin/m4 && make -j ${MAKE_JNUM} && make install
          if [ "x$?" != "x0" ] ; then
            echo FAILURE 2
            exit
          fi
