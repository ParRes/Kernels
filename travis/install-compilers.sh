...
case "$os" in
    ...
    Linux)
        ...
        case "$MPI_IMPL" in
            mpich1)
                sudo apt-get install -y cmake gfortran mpich-shmem-bin libmpich-shmem1.0-dev
                ;;
            mpich2)
                sudo apt-get install -y cmake gfortran mpich2 libmpich2-3 libmpich2-dev
                ;;
            mpich|mpich3)
                sudo apt-get install -y cmake gfortran libcr0 default-jdk
